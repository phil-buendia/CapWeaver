...
- 检索未命中时才动态生成临时 tool
- 先完成当前任务
- 任务结束后再决定丢弃、保留到当前会话、保存为 retained tool、进一步包装成 skill，或进入能力库治理
...
![CapWeaver Capability Lifecycle](nanocoder_tool_lifecycle.svg)
...
- 未命中时才生成任务级临时 tool
- 先完成任务，再做留存判断
- 最终进入丢弃、会话保留、retained tool、workflow skill、基于轨迹对已有 skill 做轻量修订，或者由 curator 给出改进/归档建议
...
| `trajectory revision` | 记录执行轨迹，并把踩坑、修正、边界情况沉淀到已有 skill |
| `curator` | 使用 rubric 审查 tool/skill，生成治理报告 |

## v0.4 更新

| 方向 | v0.3 | v0.4 |
|---|---|---|
| 长任务目标 | 记录任务轨迹 | 新增 `/goal`，跨轮保持任务方向 |
| 能力库治理 | 人工查看 | 新增 `/curator`，基于 rubric 生成治理报告 |
| 能力库清理 | 仅活跃库 | 新增 `/archive-skill`、`/archive-tool` 显式归档 |
| 写入安全 | 生成路径校验 | Python、JSON、TOML、YAML shape 写后校验 |
| 能力质量 | 创建和修订 | 评分、改进候选、归档候选、skillify 候选 |

## v0.3 更新

| 方向 | v0.2 | v0.3 |
|---|---|---|
| 记忆粒度 | capability event 统计 | event 统计 + 每个任务的 trajectory JSONL |
| skill 生命周期 | 创建和保存 skill | 创建、保存，并可基于后续执行修订 skill 说明 |
| 学习信号 | 留存和 skillification 判断 | 结合错误、用户修正、边界情况做任务后复盘 |
| 交互命令 | `/capstats` | `/capstats`、`/trajectories` |
...
/skills     查看已保存 skill
/capstats   查看能力增长统计
/trajectories 查看最近任务轨迹文件
/goal       查看/设置/清除长期目标
/curator    用 rubric 审查能力库
/archive-skill <name> 归档 skill
/archive-tool <name>  归档 retained tool
/save       保存会话历史
/sessions   查看已保存会话
/reset      重置当前对话
```
...
- 任务轨迹默认保存在 `~/.corecoder/trajectories/`。
- 能力事件默认保存在 `~/.corecoder/capability_events.jsonl`。
- curator 报告默认保存在 `~/.corecoder/curator/`。
