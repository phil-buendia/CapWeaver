...
def _confirm_tool(name: str, args: dict) -> bool:
    ...
    console.print(f"\n[bold yellow]⚠ {name}[/bold yellow] wants to execute:")
    for k, v in args.items():
        v_str = repr(v) if len(repr(v)) <= 120 else repr(v)[:120] + "..."
        console.print(f"  [cyan]{k}[/cyan] = {v_str}")

    try:
        choice = input("Allow? [y/N/always] ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        return False
...
def _repl(agent: Agent, config: Config):
    ...
    while True:
        ...
        if user_input == "/skills":
            ...
            skills = get_library().list_all()
            if not skills:
                console.print("[dim]No skills saved yet.[/dim]")
            else:
                console.print(f"[bold]Saved skills ({len(skills)}):[/bold]")
                for s in skills:
                    console.print(
                        f"  [cyan]{s['name']}[/cyan]: {s.get('description','')[:70]}  "
                        f"[dim](used {s.get('use_count',0)}x)[/dim]"
                    )
            continue
