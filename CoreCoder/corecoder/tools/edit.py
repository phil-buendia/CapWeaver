...
def _is_protected_skill_store(path: Path) -> bool:
    parts = {part.lower() for part in path.parts}
    return "skill_store" in parts
...
class EditFileTool(Tool):
    ...
    def execute(self, file_path: str, old_string: str, new_string: str) -> str:
        try:
            ...
            if _is_protected_skill_store(p):
                return (
                    "Error: direct edits inside skill_store are blocked. "
                    "Use tool_forge and the save-to-library flow instead."
                )
            if not p.exists():
